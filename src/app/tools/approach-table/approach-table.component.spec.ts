import { calculateApproachTable, approachTableTool } from './approach-table.component';

describe('ApproachTable', () => {
  it('builds 10 rows for NM distances', () => {
    const rows = calculateApproachTable(1500, 3, 'NM');
    expect(rows.length).toBe(10);
    expect(rows[0].distanceNM).toBe(1);
  });

  it('builds 20 rows for kilometer distances', () => {
    const rows = calculateApproachTable(1500, 3, 'km');
