...
export function parseCoordinateString(value: string, expectSeconds: boolean, isLat: boolean): number {
  ...
  const match = trimmed.match(
    /^(N|S|E|W)?\s*(-?\d+(?:\.\d+)?)\s*(?:[°ºd])?\s*(\d+(?:\.\d+)?)\s*'?\s*(?:(\d+(?:\.\d+)?)\s*"?)?\s*(N|S|E|W)?$/
  );
...
export class CoordinatesConversionComponent {
  ...
  convert(): void {
    try {
      ...
      switch (format) {
        case 'dd':
          pair = parseDecimalDegrees(this.latInputControl.value, this.lonInputControl.value);
          break;
        case 'dm':
          pair = parseDegreesMinutes(this.latInputControl.value, this.lonInputControl.value, false);
          break;
        case 'dms':
          pair = parseDegreesMinutes(this.latInputControl.value, this.lonInputControl.value, true);
          break;
        case 'mgrs':
          pair = parseMgrs(this.mgrsInputControl.value);
          break;
        default:
          throw new Error('Unsupported format selected.');
      }
      this.results.set(buildCoordinateResults(pair));
